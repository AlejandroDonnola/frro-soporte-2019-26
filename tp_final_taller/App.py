...
def controladorfactuahojas():
    if request.method == 'POST':
        ...
        return render_template('form_hoja.html',id_factura=id_factura,id_cliente=f.id_usuario,id_patente=a.id_patente,mecanicos=negocio.lista_mecanicos())
...
def controladorrepuesto():
    if request.method == 'POST':
        ...
        descripcion=r[int(select)-1][1]
        idr=negocio.agregar_repuesto_importado(descripcion[:10],r[int(select)-1][2])
