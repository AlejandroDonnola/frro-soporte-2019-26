...
def lista_facturas():
    ...
    if(facturas!=False):
        ...
        for f in  facturas:
            ...
            lista_facturas.append((f.id_factura,f.fecha_emision, str(x.apellido)+', '+str(x.nombre),x.dni,f.importe_total))
        return lista_facturas
    else:
        return False
...
def lista_mecanicos():
    mecanicos = session.query(datos.Usuario).filter(datos.Usuario.id_rol == 2).all()
    if(mecanicos!=False):
        lista_mecanicos=[]
        for m in mecanicos:
            lista_mecanicos.append((m.id_usuario,str(m.apellido)+', '+str(m.nombre),m.dni,m.email))
